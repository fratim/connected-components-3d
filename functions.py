    ...
    for bz in range(z_start, z_start+n_blocks_z):
        ...
        for by in range(y_start, y_start+n_blocks_y):
            for bx in range(x_start, x_start+n_blocks_x):
                ...
                input_name = "/z"+str(bz).zfill(4)+"y"+str(by).zfill(4)+"x"+str(bx).zfill(4)+"/" + "filled"
...
class dataBlock:
    ...
    def readLabels(self, data_path, sample_name, bz, by, bx):
        if param.isCluster:
            ...
            filename = data_path+"/"+sample_name+"/"+"Zebrafinch-input_labels_discarded-"+str(bz).zfill(4)+"z-"+str(by).zfill(4)+"y-"+str(bx).zfill(4)+"x"
        else:
