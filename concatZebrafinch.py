...
from funtions import readData, makeFolder, blockFolderPath, writeData
...
for bz in zrange[::4]:
    ...
    filename = param.folder_path+"/"+sample_name+"/"+str(bz*128).zfill(4)
    block_a = readData(box=[1],filename=filename)

    if bz != zrange[-1]:
        filename = param.folder_path+"/"+sample_name+"/"+str(bz+1*128).zfill(4)
        block_b = readData(box=[1],filename=filename)
        block_a = np.concatenate((block_a,block_b), axis=0)
        del block_b

        filename = param.folder_path+"/"+sample_name+"/"+str(bz+2*128).zfill(4)
        block_c = readData(box=[1],filename=filename)
        block_a = np.concatenate((block_a,block_c), axis=0)
        del block_c

        filename = param.folder_path+"/"+sample_name+"/"+str(bz+3*128).zfill(4)
    ...
    for y_block in range(3):
        for x_block in range(3):
            ...
            writeData(param.folder_path+"/z"+str(bz).zfill(2)+"y"+str(by).zfill(2)+"x"+str(bx).zfill(2), block_a[:,y_block:(y_block+1):bs_y,x_block:(x_block+1):bs_x])
